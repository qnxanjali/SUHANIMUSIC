...
def generate_sticker(client, message):
    if len(message.command) == 2:
        ...
        message.reply_text("Please SUHANIvide a sticker ID after /st command.")
...
async def _packkang(app :app,message):  
    ...
    if len(message.command) < 2:
        pack_name =  f'SticksSuhani - @SticksSuhani'
    else :
    ...
    try:
        short_name = f'SticksSuhani_{str(uuid4()).replace("-","")}_by_{app.me.username}'
